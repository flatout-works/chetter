...
The trigger is disabled by default. Enable privileged MCP profiles only after installing the GitHub App on the target repository and making `CHETTER_MCP_AUTH_TOKEN` available to trusted runners. Credentialed profiles are trusted/admin-only in this MVP; `tool_allowlist` is not server-side security enforcement, and resolved credentials are written into task-readable harness config files. Until scoped MCP tokens or proxy-side enforcement exists, do not use privileged Chetter MCP profiles for untrusted or multi-tenant tasks.
