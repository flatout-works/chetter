...
The trigger is disabled by default. Enable it only after installing the GitHub App on the target repository and making `CHETTER_MCP_AUTH_TOKEN` available to trusted runners. The `chetter-orchestration` profile grants the full Chetter MCP tool surface to the trusted orchestrator task. Reviewer child tasks inherit read-only GitHub auth for cloning and `gh` inspection, but not GitHub write authorization. The orchestrator polls child tasks with text-free `chetter_task_state`; the synthesizer receives child exports as server-copied workspace files without Chetter MCP profiles or GitHub write credentials, while `definition_repo` keeps base-repo agent/skill definitions for fork PRs. The orchestrator verifies the head SHA and posts the marked final review body from the synthesizer export through `chetter_pr_review` by task ID without reading transcript contents. Until scoped MCP tokens or proxy-side enforcement exists, this workflow is for trusted self-hosted deployments.
