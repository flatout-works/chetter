...
The trigger is disabled by default. Enable it only after installing the GitHub App on the target repository and making `CHETTER_MCP_AUTH_TOKEN` available to trusted runners. Reviewer child tasks inherit read-only GitHub auth for cloning and `gh` inspection, but not GitHub write authorization; only the synthesizer should post through Chetter MCP tools. Until scoped MCP tokens or proxy-side enforcement exists, this workflow is for trusted self-hosted deployments.
