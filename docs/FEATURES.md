...
Event callbacks react to task lifecycle events with `create_task`, `webhook`, or `slack` actions, managed via `chetter_create_event_callback` and friends. All three action types run through the durable `callback_deliveries` outbox: the delivery row is written in the same transaction as its task event and executed by a leased worker with exponential backoff, retrying up to `max_attempts` (3) before dead-lettering. See [TRIGGERS.md](TRIGGERS.md#event-callbacks).
