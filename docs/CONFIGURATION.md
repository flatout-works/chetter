...
| `global/webhooks/inbound/*.yaml`, `groups/<team-name>/webhooks/inbound/*.yaml` | `name`, `auth`, `action` | Generic inbound webhook endpoint (issue #120), global or team-scoped only. The filename stem must equal `name` and unknown fields are rejected (strict schema: `schemas/inbound-webhook.schema.json`). Global endpoints must declare `action.team_name`; team-scoped endpoints inherit their team and must not declare it. `auth.secret_env` names a server environment variable — the secret value is never stored. See [WEBHOOKS.md](WEBHOOKS.md). |
