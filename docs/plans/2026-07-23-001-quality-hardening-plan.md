...
| R5 | Partially fixed | Task/execution IDs, generic workspace tools, and `ExtraFiles` writes now reject traversal and symlink escapes, but resume paths and host-path translation remain unchecked. `runner/internal/workspace/files.go`; `runner/internal/controller/runner_task.go:702-715`; `runner/internal/controller/docker_args.go:34-36`; `runner/internal/agentenv/environment.go:18-26`. | P0 shared safe-path boundary for the remaining resume and host paths. |
