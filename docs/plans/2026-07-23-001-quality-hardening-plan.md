...
| R2 | Fixed (2026-08-01) | The runner-wide MCP relay requires capabilities registered for active executions, rejects unauthorized requests before proxying, replaces the capability with the upstream token, forwards task/execution fencing headers, counts rejections, and binds to the runner interface. `runner/internal/network/mcp_relay.go`; `runner/internal/network/listen.go`. | Keep relay authentication, revocation, and upstream-isolation regression coverage. A control-plane audit event for anonymous rejection remains follow-up observability work. |
