...
| S10 | Fixed (2026-07-29) | Synced triggers carry a deterministic identity keyed on (source ID, name); `reconcileSyncedCronEntries` tears down in-memory cron schedules for removed, renamed, disabled, or no-longer-cron triggers, leaving API and other-source triggers untouched. `internal/service/model_catalog_tools.go:723-865`; regression coverage in `internal/service/trigger_sync_test.go` and `internal/service/definitions_sync_test.go`. | Keep trigger-identity and cron-reconciliation regression coverage. |
...
| S17 | Fixed (2026-09-04) | Outbound webhook delivery (event-callback `webhook`/`slack` actions) now runs through an SSRF-safe destination policy and dedicated client in `internal/ssrf/`: https-only by default (explicit `CHETTER_WEBHOOK_ALLOW_HTTP` override), loopback/link-local/private/metadata/reserved destinations rejected unless explicitly allowed by operator policy (`CHETTER_WEBHOOK_ALLOW_PRIVATE`, `CHETTER_WEBHOOK_ALLOWLIST`), every dialed address checked with no DNS-rebinding window, no proxy env vars, no redirects, bounded timeouts, create/update-time validation plus audit-logged rejections. See issue #337. | Keep policy and dial-time regression coverage. |
...
| R2 | Fixed (2026-08-01) | The runner-wide MCP relay requires capabilities registered for active executions, rejects unauthorized requests before proxying, replaces the capability with the upstream token, forwards task/execution fencing headers, counts rejections, and binds to the runner interface. Heartbeats carry cumulative rejection counts; the control plane persists count increases as system-scoped audit events and exports an aggregate Prometheus gauge. `runner/internal/network/mcp_relay.go`; `runner/internal/controller/heartbeat.go`; `internal/service/runner_rpc.go`; `internal/metrics/metrics.go`. | Keep authentication, revocation, upstream-isolation, audit-delta, restart/reset, and metric regression coverage. |
| R3 | Fixed (2026-08-01) | Per-execution MCP servers use fresh 256-bit bearer capabilities, bind to loopback or the runner interface, inject authenticated config into all five harnesses, use owner-only config files, and redact capabilities from runner output. `runner/internal/mcp/server.go`; `runner/internal/controller/runner_task.go`; `runner/harness/*/config.go`. | Keep unauthenticated/authenticated, harness config, permission, serialization, and redaction regression coverage. |
| R4 | Fixed (2026-08-03) | Runners advertise enforced isolation (gVisor/runsc available and active) in claim/heartbeat metadata; server admission only places isolation-requiring tasks (resumable, explicit `isolation: required`, or every task in a hardened fleet) on capable runners and fails them fast (`isolation_unavailable`) when none exist; runners refuse isolation-requiring tasks at claim time when they cannot enforce a sandbox; `CHETTER_ALLOW_UNISOLATED` is the documented escape hatch for trusted single-tenant deployments. `runner/internal/controller/isolation.go`; `internal/service/runner_rpc.go`; `db/migrations/051_add_isolation_columns.sql`. | Keep policy, admission, fail-fast, and escape-hatch regression coverage. |
| R5 | Fixed (2026-08-02) | Task/execution IDs, generic workspace tools, `ExtraFiles` writes, persisted resume workspaces, Docker bind-mount translation, and workspace cleanup now enforce manager-owned path boundaries and reject symlink escapes. `runner/internal/workspace/manager.go`; `runner/internal/workspace/files.go`; `runner/internal/agentenv/environment.go`; `runner/internal/controller/docker_args.go`. | Keep traversal, symlink, resume, bind-mount, and cleanup regression coverage. |
| R6 | Fixed (2026-08-15) | Drain now runs a completion barrier after the deadline expires: the runner blocks exit until every task's teardown (sandbox teardown, workspace destroy, session-export flush) and terminal report complete, bounded by `CHETTER_DRAIN_HARD_KILL_TIMEOUT_SEC` (default 60s); a hard-kill audit log lists in-flight executions and report-delivery status. `runner/internal/controller/heartbeat.go`; `runner/internal/controller/runner.go`; `runner/internal/controller/runner_rpc.go`; `runner/internal/controller/runner_task.go`; tests in `runner/internal/controller/runner_drain_test.go`. See issue #313. | Keep barrier regression coverage. |
| R7 | Partially fixed | Process-group cancellation and process waiting improved, and terminal reporting is now joined to the drain barrier and clamped to the hard-kill budget (issue #313). RPC writes can still block. `runner/internal/controller/process_unix.go:10-21`; `runner/internal/controller/runner_task.go:1573-1585`; `runner/internal/controller/runner_rpc.go:216-232`. | P1 bounded writer. |
...
| R9 | Partially fixed | Cleanup and cancellation ordering improved, and drain now blocks exit until deferred cleanup and terminal publication complete (issue #313). Terminal publication still precedes some deferred cleanup (container removal), but the barrier guarantees the runner never exits mid-cleanup. `runner/internal/controller/runner_task.go:742-747,918-924,1435-1437`; `internal/service/api.go:154-178`. | P1 execution lifecycle barrier (report-before-teardown ordering). |
...
| R13 | Fixed (2026-08-15) | Terminal reports are tracked on a report barrier (reportWG) joined by the drain cleanup phase; retries are budget-bounded to the hard-kill deadline so a blocked report cannot outlive the runner. `runner/internal/controller/runner_rpc.go`; tests in `runner/internal/controller/runner_drain_test.go`. See issue #313. | Keep shutdown-join regression coverage. |
