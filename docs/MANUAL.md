...
| `CHETTER_LOG_LEVEL` | No | `info` | Minimum structured log level: `debug`, `info`, `warn`, or `error` (case-insensitive). Invalid values fail clearly at startup. See issue #87. |
| `CHETTER_LOG_FORMAT` | No | `text` | Log output format: `text` (human-readable key=value) or `json` (one JSON object per line). Invalid values fail clearly at startup. See issue #87. |
