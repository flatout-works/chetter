...
After the drain deadline expires the runner enters a **forced-cleanup completion barrier**: it blocks exit until every force-cancelled task's teardown — sandbox teardown, workspace destroy, checkpoint/session-export flush — and its terminal `ReportTaskEvents` delivery have completed, so a rolling deploy never exits mid-cleanup or loses an in-flight terminal result. The barrier is bounded by `CHETTER_DRAIN_HARD_KILL_TIMEOUT_SEC` (default `60s`); terminal-report retries are clamped to the same budget so reporting provably finishes before exit. When the hard-kill timeout fires, the runner logs each still-in-flight execution with its terminal-report delivery status (and exits 1). Keep drain timeout + hard-kill timeout below the pod's `terminationGracePeriodSeconds` — the supplied deployment manifests set `terminationGracePeriodSeconds: 120` for 60s drain + 30s hard-kill + margin. Exits 0 on a clean drain, 1 if tasks were force-cancelled. See issue #313.

### Kubernetes disruption protection

The `deploy/k8s/` kustomization ships a `PodDisruptionBudget` (`poddisruptionbudget.yaml`, `minAvailable: 1`) that gates **voluntary** evictions of the runner pod — node drains, cluster-autoscaler scale-down, cluster upgrades — so Kubernetes "waits out" in-flight work instead of tearing down the pod. The runner Deployment additionally sets `terminationGracePeriodSeconds: 120` (headroom above the drain budget), a `maxSurge: 1 / maxUnavailable: 0` rolling strategy, best-effort `topologySpreadConstraints` across nodes, and the `cluster-autoscaler.kubernetes.io/safe-to-evict: "false"` annotation. See [EKS.md](EKS.md#step-7-deploy-the-runner-kubernetes-mode) for node-maintenance workflow and limitations.

**Limitations:** the PDB covers only voluntary disruption. Node failure, OOM kills, and spot reclamation terminate the runner pod regardless, and Kubernetes garbage collection then deletes the agent Pods it owns (`chetter.io/owned=true`), killing in-flight work. The task is re-queued by the reaper after lease expiry and restarts on another runner; resumable sessions resume from their checkpoint.
