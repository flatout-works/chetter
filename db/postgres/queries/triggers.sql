...
SELECT * FROM triggers WHERE enabled = true ORDER BY created_at DESC;

-- name: ListTriggersByTeam :many
SELECT * FROM triggers WHERE team_id = sqlc.arg(team_id) ORDER BY created_at DESC;
...
DELETE FROM triggers WHERE source_id = $1;

-- name: SetTriggerLastRun :exec
UPDATE triggers SET last_run_at = $1, updated_at = $2 WHERE id = $3;
