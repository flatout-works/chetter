...
func TestExampleMCPProfileDefinitionsParse(t *testing.T) {
	...
	if !containsString(profile.ToolAllowlist, "chetter_submit_task") || !containsString(profile.ToolAllowlist, "chetter_pr_review") {
		t.Fatalf("example profile is missing expected Chetter tools: %#v", profile.ToolAllowlist)
	}

	triggerData, err := os.ReadFile(filepath.Join(root, "triggers", "example-daily-review.yaml"))
	if err != nil {
	...
	if trigger.TriggerType != "cron" || trigger.Agent != "example" {
		t.Fatalf("unexpected trigger: %+v", trigger)
	}
	if trigger.Enabled {
		t.Fatal("example trigger should remain disabled by default")
	}
	if !containsString(trigger.Skills, "example") {
		t.Fatalf("trigger skills = %#v, want example", trigger.Skills)
	}
	if len(trigger.MCPProfiles) != 0 {
		t.Fatalf("example trigger should not enable privileged mcp_profiles by default: %#v", trigger.MCPProfiles)
	}
}
