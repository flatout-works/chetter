...
func TestGHWrapperBlocksWritesAfterGlobalFlags(t *testing.T) {
	tests := [][]string{
...
func TestGHWrapperAllowsReadCommandsWithGlobalFlags(t *testing.T) {
	out, err := runGHWrapper(t, "--repo", "owner/repo", "pr", "view", "123")
	if err != nil {
		t.Fatalf("expected read command to pass through: %v\n%s", err, out)
	}
	if !strings.Contains(out, "REAL --repo owner/repo pr view 123") {
		t.Fatalf("unexpected fake gh output: %s", out)
	}
}
