...
- Delivery retention for the durable delivery tables (issue #253): `DELIVERY_RETENTION_DAYS` bounds `callback_deliveries`, `inbound_deliveries`, and `webhook_deliveries` the same way `EVENTS_RETENTION_DAYS`/`AUDIT_RETENTION_DAYS`/`ARTIFACT_RETENTION_DAYS` bound their tables. Only terminal rows (`completed`, `succeeded`, `failed_permanent`, `dead_letter`) older than the TTL are deleted; `pending`, `in_flight`, `processing`, `received`, `failed`, and `retry_wait` rows are never removed, so retention cannot discard retryable work. `0` (the default) disables pruning, matching the other retention settings. Documented in `docs/MANUAL.md` and `docs/FEATURES.md`.
...
- Double execution of the same task after a lease-fence failure (the 2026-09-20 wowbagger pile-up: one runner executed two sandboxes of the same task, 7.7 GB RSS, host RAM exhaustion). `RenewExecutionAttemptLease` required `lease_expires_at > CURRENT_TIMESTAMP`, so a single missed heartbeat made the lease permanently unrenewable: every later renewal returned 0 rows and told the runner to stop, but if that cancel never landed the execution kept running while the reaper marked the attempt lost and requeued the task, and the runner then claimed the new attempt alongside the old one. The renewal gate is now status + `runner_id` + `claim_id`, all verified by the heartbeat handler against the reported execution; the reaper fences via `FOR UPDATE` on expired rows, so a committed renewal moves the lease forward and the locking read drops the row while a committed reaper marks the attempt `lost` and the renewal's status filter rejects it. A healthy runner therefore always recovers its lease after a transient gap. The runner additionally refuses to start a second execution of a task it is already executing — the new attempt fails fast — so even a control-plane bug can no longer double resource usage. `TestRPCHeartbeatRenewsExpiredExecutionStillOwnedByRunner` replaces the test that pinned the old fence.
- Two defects that kept `make check` red on main since 2026-09-08 (blocking every `arcane-build-deploy` deployment): the PostgreSQL DDL translation only rewrote `TINYINT(1) ... DEFAULT 0`, leaving `webhook_endpoints.enabled` as `BOOLEAN NOT NULL DEFAULT 1` (SQLSTATE 42804) and breaking PostgreSQL bootstrap on every startup; and the callback-delivery statements bound the wrong number of arguments — the MySQL/TiDB branch of `ListCallbackDeliveries` used a 4-placeholder status filter (`(? = '' OR status = ?) ... LIMIT ? OFFSET ?`) but bound only `statusFilter, limit, offset`, and its PostgreSQL mirror `claimDueCallbackDeliveries` repeated `$1` for all three timestamp comparisons while binding three arguments. Both are unified into one `sqlQuery`-expanded statement binding the filter once per placeholder, with regression tests for the parse blocks.

## 2026-09-20

### Fixed

- Runner container leaks that could overload a host (issue #418): task containers whose teardown failed were never retried and never reaped, so on a memory-pressured host a leaked gVisor sandbox could keep running indefinitely. The incident that prompted this (wowbagger, 2 runners on a 16 GB host) left 8 sandboxes running up to 10 days after their tasks were terminal; host load reached 115 on 8 cores, swap filled, TiKV crash-looped, and the control plane became unreachable because the MCP could not reach the database. `removeTaskContainer` and `stopTaskContainer` now retry (2 attempts, `containerCleanupTimeout` raised 30s → 120s) instead of abandoning the container when a single `docker rm -f` is killed by its context, and a container that no longer exists short-circuits via `isContainerGoneError`. A periodic reaper (5-minute interval, 10-minute minimum container age, 2-minute pass timeout) now bounds how long any `chetter-task-*` container can outlive its task, replacing the startup-only sweep that filtered to the runner's own ID and skipped containers whenever a checkpoint listing returned anything.

### Added

- Control-plane-authoritative container reaping (issue #418): because two runners can share one Docker daemon, a runner cannot locally distinguish a leaked orphan from a live sibling's sandbox. The reaper calls `PruneWorkspaces` with the new `container_scope` flag, which relaxes only the *ownership* predicates while the *liveness* predicates (running attempt, retained session, ready checkpoint) always protect live work, and it fails closed when no verdict is available. A new `chetter.workspace_path` container label (the runner-side path, matching `agent_sessions.workspace_path`) lets the server match a container to retained session state; containers predating the label submit an empty path and fall back to task-wide protection. Docker `checkpoint ls` remains a local independent guard.

### Changed

- `deploy/compose.yaml` sets runner-level sandbox caps — `CHETTER_CONTAINER_MEMORY=4g`, `CHETTER_CONTAINER_CPU=2`, `CHETTER_CONTAINER_PIDS=1024` (all env-overridable) — because `CHETTER_TASK_MAX_MEMORY_MB` is per task, not per host: two runners with the previous 8 GB per-task default could each start an 8 GB sandbox on a 16 GB host, and gVisor's sentry lives outside the container cgroup. These caps can only tighten the server's per-task limit. Documented in `runner/README.md`.

## 2026-09-18

### Changed

- `create_task` event callbacks are now durable (issue #405, merged in #414), removing the last at-most-once callback path. `webhook` and `slack` actions already wrote a `callback_deliveries` row in the same transaction as their task event, but `create_task` spawned its child post-commit with only a logged warning on failure, so a replica crash or a failed submission between the event commit and the spawn silently lost the spawned work. All three action types now enqueue in that same transaction and are executed by the leased multi-replica delivery worker, so a persisted event can no longer lack its delivery record. New `action_type` and `child_task_id` columns select the execution path and record the spawned task (Goose migrations 057 for MySQL/TiDB and 033 for PostgreSQL, with `action_type` backfilled from `event_callbacks`); the child task id is derived deterministically from the delivery row, so a replay after a crash adopts the existing child instead of duplicating it, and the callback recursion-depth guard (#312) is re-checked at execution time with terminal failures dead-lettered and audited. `chetter_list_callback_deliveries` now reports all three action types. Documented in `docs/TRIGGERS.md`, `docs/SCHEMA.md`, `docs/MANUAL.md`, and `docs/PLAN.md`.
...
- Durable outbound delivery queue for event callbacks (issue #357, merged in #394): `webhook` and `slack` callback actions were previously dispatched synchronously and in-process, so a transient network failure or non-2xx response dropped the notification permanently (failures were logged only). Every matching event now writes a `callback_deliveries` row (unique per `(callback_id, event_id)`) in the same transaction as its task event at all event insert/dispatch sites (task claim, `recordTaskEvent`, lease-reaper reclaim, task recovery), so a persisted event can never lack its delivery record; post-commit dispatch was limited to the synchronous `create_task` action — the last at-most-once path, removed on 2026-09-18 (see the 2026-09-18 entry above). A leased multi-replica delivery worker claims due rows with `FOR UPDATE SKIP LOCKED` under a 60-second lease (expired `in_flight` leases are reclaimed after a crash) and delivers each attempt through the SSRF-safe client from #337, retrying failures with exponential backoff (1s/5s/15s/30s) up to `max_attempts` (3) before dead-lettering. Retry, dead-letter, and completion transitions are audited (`callback_delivery_failed`, `callback_delivery_dead_letter`, `callback_delivery_completed`), and the new `chetter_list_callback_deliveries` MCP tool lists delivery records (`pending`, `in_flight`, `completed`, `failed`, `dead_letter`) with attempts, next attempt time, and error details. Schema shipped in Goose migrations 055 (MySQL/TiDB) and 031 (PostgreSQL) plus `internal/store/schema.go` bootstrap; the `webhook_deliveries` table continues to cover only inbound GitHub webhook processing. Documented in `docs/TRIGGERS.md`, `docs/SCHEMA.md`, and `docs/PLAN.md`.

- Generic inbound webhook endpoints (issue #120, merged in #395): a new `inbound_webhook` definition type scans `global/webhooks/inbound/*.yaml` and `groups/<team>/webhooks/inbound/*.yaml` with a strict (unknown-field-rejecting) schema, fixed team-ownership rules, filename/name matching, and secrets referenced by environment variable name only (`auth.secret_env`) — never inlined. Definition sync materializes endpoints into `webhook_endpoints` rows with stable opaque public ids (removing a file removes the endpoint), backed by new `webhook_endpoints` and `inbound_deliveries` tables (Goose migrations 056 MySQL/TiDB and 032 PostgreSQL plus `internal/store/schema.go` bootstrap). External systems POST authenticated events to `POST /hooks/inbound/<public_id>`, which enforces content type, a body-size limit (default 1 MiB), per-endpoint fixed-window rate limiting (default 60 requests/minute), a per-endpoint backlog bound (default 200 non-terminal deliveries, answering 429 above it), HMAC-SHA256 or bearer auth from `secret_env`, and delivery-id replay protection, and returns 202 only after the delivery row is durably inserted. A leased delivery worker claims rows atomically (`FOR UPDATE SKIP LOCKED`), recovers stale leases, and performs one `create_task` action per delivery with a deterministic task id so retries can never duplicate tasks; delivery statuses are `pending`, `processing`, `succeeded`, `retry_wait`, `failed_permanent`, and `dead_letter`, with audit events that never carry payload or secret material. New team-scoped MCP tools `chetter_list_inbound_endpoints` (public URL, auth type, secret availability, action, team scope; secret values never returned) and `chetter_list_inbound_deliveries` (statuses, retry attempts, linked task, error text; payloads never returned). Ships a `schemas/inbound-webhook.schema.json` JSON schema, an example `global/webhooks/inbound/ci-build-events.yaml` config, docs (`docs/WEBHOOKS.md`, `docs/SCHEMA.md`), and tests (definition parsing, receiver unit tests, DB integration tests).
...
- SSRF-safe destination policy for outbound delivery (issue #337, merged in #382): event-callback webhook and Slack destinations are validated before use and re-checked on every dial, so a callback cannot be aimed at loopback, link-local, private, or cloud-metadata addresses. HTTPS is required by default (`CHETTER_WEBHOOK_ALLOW_HTTP=true` permits plain HTTP), private, link-local, and metadata targets are refused unless `CHETTER_WEBHOOK_ALLOW_PRIVATE=true`, and `CHETTER_WEBHOOK_ALLOWLIST` restricts delivery to an explicit destination list (an invalid allowlist fails startup). Every address a hostname resolves to is checked, not just the first, so a poisoned or rebinding DNS answer cannot slip a blocked address through, and the client dials the validated address rather than re-resolving. Destinations rejected by policy are not enqueued, and a policy change made after enqueue still cannot reach a blocked target. Documented in `docs/TRIGGERS.md` and `docs/MANUAL.md`.

### Fixed

- opencode 1.0.180 config compatibility (merged in #386): the harness config now nests provider `baseURL`/`apiKey` under `options` and writes `external_directory` as `"allow"`, which are the shapes 1.0.180 requires; the previous config was rejected.
- Agent base image harness repairs (merged in #387): a dangling `pi` symlink is repaired and `@mem9/opencode` is pinned back to the loader-compatible `0.1.3`, so OpenCode tasks stop failing at startup. The pin's rationale is recorded in `docs/MANUAL.md`.

## 2026-09-03

### Added

- Configurable structured logging and request IDs (issue #87, merged in #377): `CHETTER_LOG_LEVEL` (`debug`/`info`/`warn`/`error`, default `info`) and `CHETTER_LOG_FORMAT` (`text`/`json`, default `text`) configure the server's slog output at startup, with invalid values failing config validation instead of silently mis-logging. New `internal/requestid` middleware assigns every HTTP request a bounded, sanitized `X-Request-ID` — a valid inbound value is propagated, otherwise one is generated at ingress — returned in the response, stored in the request context, and logged on the ingress line. Both HTTP servers (MCP + runner RPC + webhook, and web API + UI) are wrapped, `/healthz`, `/readyz`, and `/metrics` probes log at debug to avoid noise, and webhook ingress plus async delivery logs carry the request ID so HTTP-level and async processing logs correlate. Task lifecycle logs in the runner RPC path now include `task_id` and `execution_id` alongside the existing `runner_id`/`session_id`. Documented in `docs/MANUAL.md` and `docs/BACKEND.md`.
...
- Configurable structured logging and request correlation IDs (issue #87): `CHETTER_LOG_LEVEL` (`debug`/`info`/`warn`/`error`, default `info`) and `CHETTER_LOG_FORMAT` (`text`/`json`, default `text`) configure the server's slog output at startup; invalid values fail startup with a clear error via config validation instead of silently mis-logging (`internal/logging` builds the handler). A new `internal/requestid` middleware assigns every request a bounded, sanitized `X-Request-ID` — a valid inbound value (≤64 alphanumeric, dash, or underscore characters) is propagated, otherwise a `req_`-prefixed ID is generated at ingress — echoed in the response, stored in the request context, and logged on the ingress line, with `/healthz`, `/readyz`, and `/metrics` probe requests logged at debug to avoid noise. Both HTTP servers (MCP + runner RPC + webhook, and web API + UI) are wrapped, webhook ingress and async delivery logs carry the request ID so HTTP-level and async processing logs correlate, and task lifecycle logs in the runner RPC path now include `task_id` and `execution_id` alongside the existing `runner_id`/`session_id`. Documented in `docs/MANUAL.md` and `docs/BACKEND.md`.
...
- Database-only multi-replica coordination: the server now supports horizontal scaling across replicas without Redis or external pub/sub. Five tiers ensure correctness: claim wake-up via a polled `claim_notify_counter` table (~1s, replacing the single-replica in-process-only notifier), cron deduplication via `trigger_locks` rows locked with `SELECT ... FOR UPDATE` plus a persisted `last_triggered_at` interval marker (and a re-checked `enabled` flag, so enable/disable/delete converge within one tick), strict pending admission via an `admission_locks` row lock in a `READ COMMITTED` transaction (concurrent replicas cannot overshoot `CHETTER_MAX_PENDING_TASKS`), durable `runner_drain_requests` delivered at-least-once on every heartbeat until the runner reports a draining status (the ack) with unacknowledged rows dropped after 30 minutes (`drain_runner` fails closed on DB errors), and cross-replica event catch-up where task-event and fleet streams poll the DB every 1s alongside the local event bus. Adds coordination schema + seed rows, Goose migrations 054 (MySQL/TiDB) and 030 (PostgreSQL), a migration regression test, and integration tests for cross-replica claim wake-up, drain delivery/ack/TTL, redaction, and strict admission. Persisted task-event payloads are now snapshotted after redaction scrubbing (a pre-redaction snapshot could previously leak). Streams degrade gracefully on transient DB errors instead of disconnecting viewers. `AGENTS.md`, `docs/BACKEND.md`, and `docs/DEPLOYMENT.md` document the coordination model and its limitations (per-replica redaction, timestamp-based event cursors, eventually consistent trigger toggles, at-least-once drain delivery).
- Security hardening for the web UI, webhook, runner, and deployments: OIDC sessions are HS256-only with issuer and required-expiry validation, use config-derived secure cookies (`__Host-chetter-session` on HTTPS, `SameSite=Strict`), require a minimum 32-byte session secret, and no longer embed the raw upstream ID token; logout uses the configured origin and `client_id`, never request headers; OIDC login/callback endpoints are per-IP rate limited with `Retry-After`; responses carry a CSP with script hashes computed from the served `index.html` (no `unsafe-inline`), X-Frame-Options, nosniff, Referrer/Permissions-Policy, and HSTS on TLS; `/api/server-info` exposes only `oidcEnabled`/`allowTokenLogin` publicly; `CHETTER_ALLOW_TOKEN_LOGIN` disables browser token login (frontend loads server capabilities before honoring URL/`localStorage` tokens); optional bearer auth for `/metrics` via `CHETTER_METRICS_AUTH_TOKEN`; HTTP read/idle/header limits on both servers; the webhook rejects non-POST methods; runner config is written under umask 077; Docker task containers run with `--cap-drop ALL` and no-new-privileges; K8s task pods drop all capabilities with no privilege escalation; the MCP deployment runs non-root; Compose binds to `127.0.0.1` by default; the agent base image pins and SHA-256-verifies the gh, Node, and OpenCode downloads. `docs/DEPLOYMENT.md` and `docs/MANUAL.md` document the new settings.
- Claude Code harness improvements (phases 1–4, see `docs/plans/2026-08-26-001-claude-code-harness-improvements-plan.md`): agent and skill definitions are now injected into `.claude/agents/` and `.claude/skills/` (shared `skilltar` package with the OpenCode harness); agent personas are appended via `--append-system-prompt-file` instead of replacing Claude Code's built-in prompt; cache tokens are accounted (`usage.cache_read_input_tokens`/`usage.cache_creation_input_tokens`); a session status probe (`GET /session/{id}/status`) and idle-session continuation (`POST /session/{id}/continue`, resume-based) let the progress watchdog tell hung generations from finished turns and continue idle sessions; subagent text and hook lifecycle events stream into live progress (`--forward-subagent-text`, `--include-hook-events`); `MAX_MCP_OUTPUT_TOKENS` defaults to 50000 (overridable via `CHETTER_CLAUDE_MAX_MCP_OUTPUT_TOKENS`); `--max-turns` is raised to 500 with a `CHETTER_CLAUDE_MAX_TURNS` override; an optional per-task spend ceiling is available via `CHETTER_CLAUDE_MAX_BUDGET_USD`; `--permission-mode dontAsk` plus strict MCP loading (`--mcp-config .claude/chetter-mcp.json --strict-mcp-config`) stop a cloned repository from registering its own MCP servers; `--add-dir /tmp` matches OpenCode's scratch allowance; provider errors are preserved instead of being reported as success. `docs/HARNESSES.md` documents the new capabilities.
- Pi harness improvements (phases 1–4, see `docs/plans/2026-08-26-002-pi-harness-improvements-plan.md`): the RPC path now parses the cumulative usage object Pi serializes at the top level of every `message_update` event (`input`/`output`/`cacheRead`/`cacheWrite`/`reasoning` plus `cost.total` in dollars) into `task.TokenUsage`, with deltas against a per-message baseline so interrupted streams never double-count and follow-up messages in the same session contribute their own totals; all terminal publish sites report the accumulated usage instead of `task.TokenUsage{}`. Keepalive progress is published during silent stretches (after 3s without a detail: "pi: running", "pi: running (tool in progress)", or "pi: retrying") so a live task no longer looks stuck. A requested provider/model is verified against Pi's `get_available_models` RPC response right after the readiness probe, failing fast on a model the running Pi build lacks (fail-open by design — a failed probe only warns). The documented meaning of `--approve` is corrected: it is Pi's project-trust override (`projectTrustOverride`), not a tool-approval bypass; in headless RPC mode extension UI requests are auto-answered with `cancelled: true`. Git-backed skill definitions are extracted with the shared `skilltar` package into `.pi/skills/<name>/`, and a task agent definition is written to `.pi/agent/system-prompt.md` (0600) and passed via `--system-prompt` so the persona composes with Pi's coding-assistant default prompt. The progress watchdog is now wired into the single-threaded RPC event loop: `get_state` probes run on their own goroutine and are routed back through an RPC probe hub, classifying `isStreaming`/`isCompacting`/`pendingMessageCount` into busy/idle (a busy harness is tolerated for quiet reasoning; only an idle agent advances to continuation/failure), and stalled tasks are nudged with Pi's native post-turn `follow_up` command instead of a resume-based continue — stuck cancellation interrupts the blocking line read and reports a quantified "stuck harness" error. `thinking_start`/`thinking_delta` events publish `pi: thinking…` progress (reasoning tokens counted separately via `usage.reasoning`), and runner-owned config files (`settings.json`, `.pi/settings.json`, `models.json`, `.mcp.json`) are all written mode 0600 with a regression test pinning the headless settings shape. `docs/HARNESSES.md` documents the new capabilities.
...
- Claude Code harness headless hardening (Phase 1): the proxy now parses the documented `errors[]` array on result error arms (`error_max_turns`, `error_during_execution`, `error_max_budget_usd`, `error_max_structured_output_retries`) so max-turns and budget failures surface their real message instead of a subtype fallback; `--max-turns` was raised from 100 to 500 with a `CHETTER_CLAUDE_MAX_TURNS` override while timeout, watchdog, and API retries still bound runaway loops; the proxy passes `--permission-mode dontAsk` (the documented fail-closed headless mode) instead of the undocumented `skipPermissionsOnAllowed` key; it writes `.claude/chetter-mcp.json` (mode 0600) and passes it via `--mcp-config --strict-mcp-config` so a cloned repository cannot register its own MCP servers (which `-p` mode would otherwise load without approval — skipped entries already fail via `system/init` `mcp_server_errors`); it adds `--add-dir /tmp` for scratch-space parity with OpenCode; and `.claude/settings.json` is written with mode 0600. `docs/HARNESSES.md` documents the `dontAsk` permission policy and strict MCP loading.
- Claude Code harness: provider errors are preserved instead of reporting "Claude result reported success". Claude Code returns `is_error:true` with the provider failure text in the `result` field, which the proxy previously ignored in favor of the contradictory `subtype: 'success'`, misreporting 429 rate limits and other provider failures. The proxy now extracts errors in priority order (error field, login heuristics, message, result, retry metadata, subtype), tracks structured `system/api_retry` events (attempt, max_retries, error_status, error category) as a fallback when the terminal result has no error text, detects non-empty `mcp_server_errors` in `system/init` so a failed runner-bridge or Chetter MCP server surfaces immediately, fails closed when Claude exits cleanly with no result event, and bounds error messages at 4 KB with UTF-8-safe truncation. The harness replaces bare `system.api_retry` with full rate-limit detail and decodes the proxy JSON error envelope so rate limits map to `model_error`.
- Runner/deployment security hardening: runner config is written under umask 077; Docker task containers run with `--cap-drop ALL` and `no-new-privileges`; Kubernetes task pods drop all capabilities, disable privilege escalation, disable service-account-token automount, and the MCP pod runs non-root with 1 replica; the compose stack binds to `127.0.0.1` by default. `gh`, Node, and OpenCode downloads are pinned and SHA256-verified — Node is now downloaded directly from nodejs.org tarballs with checksum verification instead of piping the NodeSource setup script through bash (the Node checksum arg naming was fixed and `.tar.xz` checksums used).

### Documentation

- Agent base image harness/CLI versions bumped and previously unpinned packages pinned: claude-code 2.1.245, codewhale 0.9.11, codex 0.149.1, pi-coding-agent 0.84.3, pi-mcp-adapter 2.27.0, @mem9/opencode 0.1.6, @opencode-ai/plugin 1.18.23, zod 4.4.3, gh 2.98.0, docker 29.7.2. claude-code and pi-coding-agent are now pinned via build `ARG`s for reproducibility. `docs/HARNESSES.md` pinned-version references and the runner daemon Dockerfile docker version were updated.
- New `docs/plans/2026-08-26-001-claude-code-harness-improvements-plan.md` records the Claude Code harness improvement plan (correctness/security hardening, OpenCode parity, observability flags, operational knobs), now marked as implemented; `docs/DEPLOYMENT.md` and `docs/MANUAL.md` cover the web UI/API security hardening config.
...
- Live execution telemetry on the task detail page (#319): a new Execution card shows the task's current runner (matched via `currentTaskIds`) with live telemetry from `RunnerFleetHealth` — CPU/memory/disk gauges, a sandbox availability badge, sandbox timing/peak stats, sandbox counters, and runner container caps. The card renders only when the runner reports non-empty telemetry and refreshes every 10s while the task runs; `GetRunnerHealth` failures stay graceful no-ops.
- Startup preflight checks for the UTC session time zone and fail-closed dialect detection (#316): `ApplySchema` now verifies the effective TiDB/MySQL session time zone is UTC before the server starts, rejects explicit non-UTC DSN parameters rather than overriding them, fails closed when dialect auto-detection probing errors, forces UTC on TiDB/MySQL connections via the DSN `time_zone='+00:00'`, and exposes the verified zone via `/readyz` and `/api/server-info` (`dbSessionTimeZone`, `dbGlobalTimeZone`, `dbTimeZoneUTC`). PostgreSQL is exempt because `TIMESTAMPTZ`/`NOW()` interval arithmetic is zone-independent.
- Session lifecycle GC for expired agent sessions (#299): `ExpirePausedSessions` now skips sessions with a pending, claimed, or running user prompt or execution attempt (in-flight fencing), so a just-resumed or actively-executing session survives even past its pause TTL; expiry and artifact GC record `session.expired` / `session.artifact_gc` audit events when they act; and Prometheus exposes `chetter_sessions{status=...}` gauges across all session statuses, including the expired bucket.

### Fixed

- Runner drain completion barrier (#313): after the drain deadline expires the runner force-cancels in-flight tasks and then blocks exit until their teardown (sandbox teardown, workspace destroy, checkpoint/session-export flush) and terminal `ReportTaskEvents` delivery complete, bounded by the new `CHETTER_DRAIN_HARD_KILL_TIMEOUT_SEC` (default 60s), instead of returning after the fixed cleanup grace and losing terminal results mid-report during rolling deploys. In-flight `runTask` goroutines and terminal reports are tracked on `taskWG`/`reportWG`, the terminal-report retry loop is clamped to the remaining hard-kill budget, and a hard-kill audit log lists in-flight executions with their report-delivery status. Replaces `CHETTER_DRAIN_CLEANUP_GRACE_SEC`.
- Runner progress watchdog: real harness progress now resets the per-stall continuation-prompt streak (3 prompts), so a harness that stalls, recovers, and stalls again later gets a fresh budget instead of being killed minutes into an otherwise healthy stall; a new absolute cap of 10 continuation prompts bounds harnesses that only move when prodded. A new `SessionStatusProbe` (implemented by the OpenCode harness via `/session/status`) makes the watchdog skip nudging and tolerate up to 12 minutes of silence while the session reports an in-flight busy turn, instead of killing a slow but live generation. Terminal stuck errors now carry quantified diagnostics (silence duration, prompt totals, last event, turn state).
- Claude Code harness: a "Not logged in · Please run /login" stream-json result — which Claude Code prints as a successful turn when no credential is available — is now classified as a run error, so such tasks fail instead of completing with that text as their summary.
- Claude Code harness: the Bash permission model changed from a per-binary allowlist to a bare `Bash` allow with a narrow deny list (AskUserQuestion plus container-escape and host-control commands such as docker, podman, systemctl, journalctl, sudo, ssh, scp, pkill, kill, shutdown, reboot). Because interactive approval prompts are unreachable in `-p` headless runs, every "ask" became a silent "deny", and Claude's Bash matcher randomly rejected compound commands, redirects, and command substitution mid-task. Deny is evaluated before allow, and the gVisor task container is the actual security boundary. Documented in `docs/HARNESSES.md`.
- CodeWhale harness: token-level event deltas are now coalesced into incremental three-second chunks instead of persisting one `task.progress` event per streamed token (which flooded the distilled timeline with fragments), keeping only `agent_message` text in the final task summary. Synthetic models previously could not run at all — the harness now generates a private custom openai-compatible provider config under `.codewhale` that keeps the API key in the runner environment (`SYNTHETIC_API_KEY`) by reference and points `CODEWHALE_CONFIG_PATH` at it. Thread creation no longer overrides the configured model with the fallback ID.
- Task listings no longer fail the whole query when any task in the result set has no agent session row (legacy tasks predating the `agent_sessions` table): session-derived fields stay empty and a warning is logged instead of returning `sql.ErrNoRows`.
- PostgreSQL exempted from the UTC session startup gate (#316): `Store.Open` skips `VerifyUTCSession` for the postgres dialect (the observed zone is captured for observability only), `/readyz` gates the UTC recheck on non-PostgreSQL, and `server-info` reports exactly the zone verified at startup. `docs/TIDB-WOWBAGGER.md` and `AGENTS.md` now scope the guard to TiDB/MySQL.
- CI deploys serialized (#328): the `arcane-build-deploy` job now runs under a `chetter-deploy` concurrency group with `cancel-in-progress: false`, so concurrent workflow runs for merged PRs queue instead of racing Arcane's `forceRecreate` redeploy (the 2026-08-15 outage was two runs recreating containers that were never started). `ci/drain-runners.sh` now resolves the MCP container by its compose service label (it previously looked for a container literally named `chetter-mcp`) and accepts either `MCP_AUTH_TOKEN` or `CHETTER_MCP_AUTH_TOKEN`, so the drain step actually drains.

### Documentation

- `docs/SESSIONS.md`: resume accepts `paused_waiting_review` sessions and documents the fenced resume status transition that prevents an expired session from being revived (#299); `docs/SCHEMA.md` scopes the UTC session requirement to TiDB/MySQL with the PostgreSQL exemption (#316); `docs/FEATURES.md` notes the live Execution telemetry card on the task detail page (#319); `docs/HARNESSES.md` documents the Claude Code Bash deny-list model.
- Website corrected the isolation model — the claim that per-task Docker networks block lateral movement was removed (the legacy bridge/netns/iptables code is gone); gVisor (`runsc`) is documented as the sandbox boundary with the transparent HTTP + DNS proxy as an operational egress control, not a network security boundary — and surfaced live per-task execution telemetry in the operations spec rows and the Web UI card on the technical architecture page.
