...
- Git-managed generic inbound webhook endpoints (issue #120, merged in #395): a new `inbound_webhook` definition type lets external systems — CI, monitoring, incidents, internal services — POST authenticated JSON events that Chetter durably turns into exactly one configured action (`create_task` in the first version). Endpoints are declared in `global/webhooks/inbound/*.yaml` and `groups/<team>/webhooks/inbound/*.yaml` with a strict schema (unknown fields rejected, filename stem must equal `name`, `schemas/inbound-webhook.schema.json`); global endpoints must declare a fixed action team, team-scoped endpoints inherit theirs from the directory, and payload data can never select the team. Definition sync materializes each file into a `webhook_endpoints` row with a stable opaque public id (removing the file removes the endpoint), and `auth.secret_env` references secrets by environment-variable name only — values are never stored, listed, logged, or returned. The receiver route `POST /hooks/inbound/<public_id>` enforces content type, body size, per-endpoint rate and backlog limits, HMAC-SHA256 or bearer auth compared in constant time, and replay protection, and returns 202 only after a durable `inbound_deliveries` insert. A leased delivery worker claims rows atomically (`FOR UPDATE SKIP LOCKED`), recovers stale leases, and performs one `create_task` action per delivery with a deterministic task id so retries can never duplicate tasks; deliveries move through `pending`, `processing`, `succeeded`, `retry_wait`, `failed_permanent`, and `dead_letter` statuses, with audit events that never include payloads or secrets. Two new team-scoped MCP tools (`chetter_list_inbound_endpoints`, `chetter_list_inbound_deliveries`) list endpoints and deliveries; secret values and payloads are never returned. Schema shipped in Goose migrations 056 (MySQL/TiDB) and 032 (PostgreSQL) plus `internal/store/schema.go` bootstrap, with an example config at `examples/config-repo/global/webhooks/inbound/ci-build-events.yaml`. Documented in `docs/WEBHOOKS.md` and `docs/SCHEMA.md`.

- Runner host-pressure claim gate (issue #397, merged in #399): before each `ClaimTask` the runner evaluates a host-pressure gate using the same memory/load sampling that feeds fleet-health telemetry. When free host memory (`MemAvailable`) drops below `CHETTER_MIN_FREE_HOST_MEMORY_MB` (default `1024` MiB, `0` disables) — or, when configured, the 1-minute load average exceeds `CHETTER_MAX_HOST_LOAD` (default `0` = disabled, because a meaningful threshold depends on the host's core count) — the runner backs off claiming instead of admitting work into a thrashing host, re-checking every few seconds. The gate is a dynamic ceiling on top of `RUNNER_MAX_CONCURRENT`, never a replacement: healthy hosts keep full configured concurrency, in-flight tasks are never cancelled or lease-dropped, and disabling the gate (`0`) preserves prior behavior. While paused the runner reports `admission_paused:memory_pressure` or `admission_paused:host_load` in its heartbeat status (visible in the Runner Fleet page and fleet API), so deliberate load shedding is distinguishable from a wedged runner that silently stopped claiming. This closes the gap left by the 2026-09-08 `RUNNER_MAX_CONCURRENT=1` default: a static ceiling limits how many tasks a runner may run, but a runner at that ceiling under sustained pressure would still claim the next task the moment a slot freed — and neither `execution.container_memory` nor the container runtime limit can see gVisor sentry overhead, which lives outside the container cgroup. Documented in `docs/MANUAL.md`, `docs/DEPLOYMENT.md`, and `runner/README.md`, with the example YAMLs and the runner JSON schema extended with the new settings.
