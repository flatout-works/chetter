...
- Pi harness improvements (see `docs/plans/2026-08-26-002-pi-harness-improvements-plan.md`): RPC mode now accounts full token usage from Pi's cumulative `message_update` usage object (input/output/cacheRead/cacheWrite/reasoning tokens plus cost), closing the last accounting gap with the OpenCode/Claude harnesses, and publishes keepalive progress during silent stretches ("pi: running", "pi: running (tool in progress)", "pi: retrying") so an alive Pi task no longer looks stuck; thinking deltas (`thinking_start`/`thinking_delta`) surface as "pi: thinking…" progress without appending to the assistant text. The progress watchdog is now wired into the single-threaded RPC event loop: it issues Pi's native `get_state` RPC probe to classify the agent busy/idle (a harness mid-generation, mid-compaction, or with queued work is tolerated for quiet reasoning; only a genuinely idle agent advances to continuation/failure) and resumes stalled tasks with Pi's native `follow_up` command using the same continuation prompt the serve-path harnesses use, so a stalled Pi task resumes without a session restart. Stuck cancellation reports a precise error with the watchdog's quantified diagnostics. Explicitly requested provider/model IDs are verified against Pi's `get_available_models` right after the readiness probe, so a request for a model absent from the running Pi build fails immediately instead of burning the timeout (fail-open when the probe itself errors; tasks relying on env or Pi defaults skip the check). Agent personas and skill definitions are now injected into Pi workspaces — the task agent definition is written to `.pi/agent/system-prompt.md` (0600) and passed via `--system-prompt` so it composes with Pi's coding-assistant default, and Git-backed skills extract with the shared `skilltar` package into `.pi/skills/<name>/` — previously silently ignored for pi tasks. Runner-owned Pi config files (`settings.json`, `.pi/settings.json`, `models.json`) are now written mode 0600, and a regression test pins the headless settings shape (`compaction.enabled`/`reserveTokens`/`keepRecentTokens`, `retry.enabled`/`maxRetries`) so a future Pi schema rename that would silently break headless behavior is caught. The documented meaning of `--approve` is corrected: it is Pi's project-trust override (projectTrustOverride), not a tool-approval bypass — in headless RPC mode there are no interactive approvals anyway. `docs/HARNESSES.md` and the comparison matrix document the new capabilities.

### Fixed

- Claude Code harness: `claude-serve-proxy` now extracts the real provider error (error field, login heuristics, message, result, retry metadata, subtype) instead of reporting the contradictory "Claude result reported success" for 429 rate limits and other provider failures, surfaces structured `system/api_retry` detail in progress (e.g. `rate_limit, HTTP 429, attempt 2/10, retry in 1.5s`), fails closed when Claude exits cleanly with no result event, detects failed runner-bridge/Chetter MCP servers via non-empty `mcp_server_errors` in `system/init`, and bounds error messages at 4 KB. The runner decodes the proxy error envelope so the error reads cleanly and rate limits classify as `model_error`.
...
- Runner agent base image: harness and tool versions updated (claude-code 2.1.245, codewhale 0.9.11, codex 0.149.1, pi-coding-agent 0.84.3, pi-mcp-adapter 2.27.0, gh 2.98.0, docker 29.7.2, `@mem9/opencode` 0.1.6, `@opencode-ai/plugin` 1.18.23, zod 4.4.3); claude-code and pi-coding-agent are now pinned via `ARG` for reproducible rebuilds.
