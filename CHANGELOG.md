...
- Pi harness improvements (phases 1–4, see `docs/plans/2026-08-26-002-pi-harness-improvements-plan.md`): Pi tasks now account token usage and cost — the cumulative usage object Pi serializes in every `message_update` event (input/output/cacheRead/cacheWrite/reasoning tokens plus `cost.total`) is parsed into `task.TokenUsage`, with deltas against a per-message baseline that resets at assistant message boundaries so interrupted streams never double-count; all terminal publish sites report the accumulated usage instead of zero. Long silent stretches (tool calls, reasoning, retries) publish `pi: running` keepalive progress after 3s instead of looking stuck. An explicit provider/model is verified against Pi's `get_available_models` RPC right after the readiness probe, so a task requesting a model absent from the running Pi build fails immediately (fail-open if the probe itself errors). Git-backed agent and skill definitions are no longer ignored for Pi: skills extract into `.pi/skills/<name>/` and the task agent persona is written to `.pi/agent/system-prompt.md` (0600) and passed via `--system-prompt`, composing with Pi's default coding-assistant prompt. The progress watchdog now probes session state via Pi's `get_state` RPC (classifying busy/idle from `isStreaming`/`isCompacting`/`pendingMessageCount`) and resumes stalled idle agents with the native `follow_up` command, tolerating quiet reasoning on busy harnesses and reporting a quantified "stuck harness" error on real stalls. `thinking_start`/`thinking_delta` events surface as `pi: thinking…` progress. Runner-owned Pi config files (`settings.json`, `.pi/settings.json`, `models.json`) are now written mode 0600, a regression test pins the headless settings shape, and the `--approve` flag is documented as Pi's project-trust override (`projectTrustOverride`) rather than a tool-approval bypass. `docs/HARNESSES.md` documents the new capabilities.
