...
func (s *Service) scanTasks(ctx context.Context, query string, args []any) ([]repository.ChetterTask, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&i.MaxAttempts, &i.RequiredRunnerID, &i.CheckpointAfterSuccess, &i.ErrorCategory, &i.SearchText,
			&i.McpProfiles,
		); err != nil {
			return nil, err
		}
