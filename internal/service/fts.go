...
func (s *Service) searchTasksRaw(ctx context.Context, teamIDs, repos []string, status, search, agent string, limit, offset int32) ([]repository.ChetterTask, error) {
	...
	if s.dialect == store.DialectMySQL {
		query = `SELECT id FROM chetter_tasks WHERE (? = '' OR status = ?) AND (? = '' OR agent = ?)` + teamClause + repoClause + ` AND MATCH(search_text) AGAINST(? IN BOOLEAN MODE) ORDER BY created_at DESC LIMIT ? OFFSET ?`
		args = append([]any{status, status, agent, agent}, teamArgs...)
		args = append(args, repoArgs...)
		args = append(args, search, limit, offset)
	} else {
		query = fmt.Sprintf(`SELECT id FROM chetter_tasks WHERE (? = '' OR status = ?) AND (? = '' OR agent = ?)%s%s AND FTS_MATCH_WORD(search_text, '%s') ORDER BY created_at DESC LIMIT ? OFFSET ?`, teamClause, repoClause, safe)
		args = append([]any{status, status, agent, agent}, teamArgs...)
		args = append(args, repoArgs...)
		args = append(args, limit, offset)
	}
...
func (s *Service) listAgentSessionsRaw(ctx context.Context, teamIDs, repos []string, status string, limit, offset int32) ([]repository.ChetterAgentSession, error) {
	teamClause, teamArgs := teamInClause(teamIDs)
	repoClause, repoArgs := repoMatchClause(repos)
	query := `SELECT id, team_id, status, resume_mode, pinned_runner_id, pinned_runner_name, checkpoint_id, workspace_path, container_name, harness_session_id, git_url, git_ref, agent_image, agent, provider_id, model_id, variant_id, created_at, updated_at, paused_at, expires_at, pause_reason, error, search_text FROM chetter_agent_sessions WHERE (? = '' OR status = ?)` + teamClause + repoClause + ` ORDER BY updated_at DESC LIMIT ? OFFSET ?`
...
func (s *Service) scanAgentSessions(ctx context.Context, query string, args []any) ([]repository.ChetterAgentSession, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			&i.ID, &i.TeamID, &i.Status, &i.ResumeMode, &i.PinnedRunnerID, &i.PinnedRunnerName,
