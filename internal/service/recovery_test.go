...
func TestRecoveryTaskRequestPreservesExecutionConfiguration(t *testing.T) {
	...
	if !reflect.DeepEqual(req.Skills, []string{"go", "review"}) || !reflect.DeepEqual(req.McpEndpoints, []string{"context"}) {
		t.Fatalf("recovery lost task dependencies: skills=%#v mcp_endpoints=%#v", req.Skills, req.McpEndpoints)
	}
	...
	if req.SubmissionSource != "recovery" {
		t.Fatalf("recovery submission source should be 'recovery': %#v", req.SubmissionSource)
	}
}
