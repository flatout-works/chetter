...
func (s *Service) createGitHubIssueTool(ctx context.Context, _ *mcp.CallToolRequest, in GitHubCreateIssueInput) (*mcp.CallToolResult, GitHubArtifactOutput, error) {
	...
	body := appendChetterSignature(in.Body, githubToolSignature(task))
...
func githubToolSignature(task repository.ChetterTask) string {
	...
	return fmt.Sprintf("---\nGenerated by [Chetter](https://github.com/flatout-works/chetter)\nAgent: %s | Model: %s | Task: %s | Runner: %s (%s)", agent, model, task.ID, runner, digest)
}
