...
func (s *Service) createGitHubIssueTool(ctx context.Context, _ *mcp.CallToolRequest, in GitHubCreateIssueInput) (*mcp.CallToolResult, GitHubArtifactOutput, error) {
	...
	body := appendChetterSignature(in.Body, githubToolSignature(task, sessionRun))
...
func githubToolSignature(task repository.ChetterTask, sessionRun repository.ChetterSessionRun) string {
	...
	if sessionRun.ID != "" && sessionRun.AgentSessionID != "" {
		return fmt.Sprintf("---\nGenerated by [Chetter](https://github.com/flatout-works/chetter)\nSession: %s\nRun: %s\nTask: %s\nAgent: %s | Model: %s | Runner: %s (%s)", sessionRun.AgentSessionID, sessionRun.ID, task.ID, agent, model, runner, digest)
	}
	return fmt.Sprintf("---\nGenerated by [Chetter](https://github.com/flatout-works/chetter)\nTask: %s\nAgent: %s | Model: %s | Runner: %s (%s)", task.ID, agent, model, runner, digest)
}
...
func stripExistingChetterSignature(body string) string {
	idx := strings.Index(body, "Generated by [Chetter](https://github.com/flatout-works/chetter)")
	if idx < 0 {
		return body
	}
	prefix := body[:idx]
	if sep := strings.LastIndex(prefix, "---"); sep >= 0 {
		return strings.TrimSpace(prefix[:sep])
	}
