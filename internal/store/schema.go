...
var schemaStatements = []string{
	...
	`CREATE TABLE IF NOT EXISTS runners (
		...
		isolation_enabled TINYINT(1) NOT NULL DEFAULT 0,
	...
	`CREATE TABLE IF NOT EXISTS task_artifacts (
		...
		agent_session_id VARCHAR(64) NULL,
		user_prompt_id VARCHAR(64) NULL,
		execution_attempt_id VARCHAR(64) NOT NULL DEFAULT '',
		...
		search_text TEXT NULL,
		PRIMARY KEY (id),
		UNIQUE KEY idx_task_artifacts_dedup (task_id, artifact_type, repo, number, execution_attempt_id),
		...
		KEY idx_task_artifacts_execution_attempt (execution_attempt_id),
	...
	// webhook_endpoints materializes inbound webhook definitions from
	// chetter-config (issue #120). Each row is the runtime identity of one
	// Git-managed inbound endpoint definition under
	// global/webhooks/inbound/*.yaml or groups/<team>/webhooks/inbound/*.yaml.
	// public_id is the opaque stable slug used in the public receiver URL
	// POST /hooks/inbound/<public_id> and never changes while the source file
	// keeps its path. Secret material is never stored: secret_env names the
	// server environment variable holding the HMAC/bearer secret.
	...
	// inbound_deliveries is the durable inbox for inbound webhook endpoint
	// requests (issue #120). A request is inserted here before the receiver
	// answers 202; a leased multi-replica worker then claims due rows and
	// performs the endpoint action (create_task) exactly once. Statuses:
	// pending, processing, succeeded, retry_wait, failed_permanent,
	// dead_letter. The unique (endpoint_id, delivery_id) key rejects client
	// replays; task_id persists the delivery/task correlation so retries can
