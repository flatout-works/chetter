// Package logging configures the server's structured slog output (issue
// #87). Operators pick a minimum level (debug/info/warn/error) and a text or
// JSON format. Invalid configuration is rejected with a clear error so
// startup fails instead of silently mis-logging.
package logging
...
// Invalid raw values (for direct binary invocations that skip Load) are
// rejected here with the same messages Validate uses.
...
// it. Call once at startup before any other logging; invalid configuration
// returns a clear error (issue #87).
