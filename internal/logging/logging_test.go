...
func TestHandlerLevelFiltering(t *testing.T) {
	logLines := func(cfg config.Logging) string {
		var buf bytes.Buffer
	...
	t.Run("info level drops debug", func(t *testing.T) {
		out := logLines(config.Logging{Format: "text", Level: slog.LevelInfo})
		if strings.Contains(out, "debug line") {
			t.Errorf("debug line should be filtered at info level: %q", out)
		}
		if !strings.Contains(out, "info line") || !strings.Contains(out, "warn line") || !strings.Contains(out, "error line") {
			t.Errorf("info/warn/error lines should be present: %q", out)
		}
	})

	t.Run("debug level keeps everything", func(t *testing.T) {
		out := logLines(config.Logging{Format: "text", Level: slog.LevelDebug})
		for _, want := range []string{"debug line", "info line", "warn line", "error line"} {
			if !strings.Contains(out, want) {
				t.Errorf("line %q should be present at debug level: %q", want, out)
			}
		}
	})

	t.Run("warn level drops info and debug", func(t *testing.T) {
		out := logLines(config.Logging{Format: "text", Level: slog.LevelWarn})
		...
		if !strings.Contains(out, "warn line") || !strings.Contains(out, "error line") {
			t.Errorf("warn/error lines should be present: %q", out)
		}
	})
}
...
func TestHandlerInvalidConfig(t *testing.T) {
	...
	t.Run("raw level overrides parsed level in Handler", func(t *testing.T) {
		var buf bytes.Buffer
		h, err := Handler(config.Logging{Level: slog.LevelInfo, LevelRaw: "warn", Format: "text"}, &buf)
		if err != nil {
			t.Fatalf("Handler: %v", err)
		}
		slog.New(h).Info("info line")
		if out := buf.String(); strings.Contains(out, "info line") {
			t.Errorf("raw warn level should filter info: %q", out)
		}
	})
}
