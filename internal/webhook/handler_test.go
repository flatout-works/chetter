...
// TestServeHTTPIncludesRequestIDInIngressLogs verifies that webhook ingress
// and async processing logs carry the HTTP request ID assigned by the
// requestid middleware (issue #87).
func TestServeHTTPIncludesRequestIDInIngressLogs(t *testing.T) {
	...
	t.Run("async processing failure log carries request id", func(t *testing.T) {
		...
		out := logBuf.String()
		// Processing fails with "missing installation.id" because the
		// handler has no github manager.
