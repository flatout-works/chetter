...
- **Multi-replica coordination**: The server supports horizontal scaling across multiple replicas using database-only coordination (no Redis or external pub/sub). Five tiers ensure correctness:
  ...
  4. **Durable drain requests** — runner drain commands are persisted in `runner_drain_requests` and delivered at-least-once: every heartbeat re-sends the command until the runner reports a draining status (the ack), and unacknowledged rows are dropped after 30 minutes. The MCP `drain_runner` tool fails closed on DB errors.
