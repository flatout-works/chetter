...
- **Single-replica assumption for task pickup**: `ClaimTask` long-polls are woken in-process by `claimNotifier` (`NotifyTaskClaimable`) the moment a task becomes claimable — `SubmitTask` (MCP tool, webhook trigger, schedule, selftest, API), `RecoverTask`, `RerunTask`, `ResumeAgentSession`, and the reaper's lease-requeue path all notify. Work that bypasses this process — submissions handled by another server replica, direct database writes, or new code paths that insert pending execution attempts without calling `s.notifyTaskClaimable()` — is only discovered by the 15s safety-net poll (`claimPollInterval` in `internal/service/runner_rpc.go`). The server must therefore run as a single replica; scaling horizontally requires replacing the in-process notifier with a shared mechanism (e.g. Redis pub/sub) and adding `NotifyTaskClaimable` calls to any new task-creation path.
