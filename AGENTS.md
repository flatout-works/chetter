...
- **TiDB must run in UTC.** All timestamps are stored as UTC and ages are computed with `TIMESTAMPDIFF(..., NOW())`. If the database's session time zone is not UTC (e.g. `@@time_zone = SYSTEM` on a host in Europe/Vienna), every age is shifted by the host offset: runners fall outside the 60s presence window and vanish from the fleet, and stale-task/reaper logic skews. The wowbagger TiDB runs `server_configs.tidb.time-zone: "UTC"` (see `docs/TIDB-WOWBAGGER.md`).
